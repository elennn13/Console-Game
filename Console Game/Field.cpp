...
Field::Field(unsigned width, unsigned height, unsigned offsetWidth, unsigned offsetHeight) {
	// генерируем значения ширины и высоты с учетом краев карты и пробелов между символами
	if (offsetWidth > 0)
	...
	for (unsigned i = 0; i < height; ++i) {
		data[i].resize(width);
		std::fill(data[i].begin(), data[i].end(), ' '); // заполнение вектора какими то значениями
...
void Field::printField() const {
	tb::place_title(0, 0, "CONSOLE_GAME", getWidth(), '-');
	tb::place_cursor(0, 1);
...
char Field::getCell(unsigned x, unsigned y) const {
